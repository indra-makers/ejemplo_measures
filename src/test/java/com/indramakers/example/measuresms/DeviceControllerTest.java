...
public class DeviceControllerTest {
    ...
    public void testCreateDevice() throws Exception {
        ...
        List<Device> devices = devicesRepository.findByName("LTS-330");
        Device device = devices.get(0);

        Assertions.assertEquals("CEN", device.getUnits());
        Assertions.assertEquals("siemens", device.getBranch());
    }

    @Test
    @Sql("/queries/test_get_devices_by_branch.sql")
    public void testGetDevicesByBranch() throws Exception {
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders
                .get("/devices/by-branch?branch=marca1")
                .contentType(MediaType.APPLICATION_JSON);
